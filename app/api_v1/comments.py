...
def get_comments():
    ...
    pagination = Comment.query.filter_by(disabled=False).order_by(Comment.timestamp.desc()).paginate(
...
def moderate_comment(id):
    comment = Comment.query.get_or_404(id)
    comment.disabled = bool(request.json.get('disabled', not comment.disabled))
    db.session.add(comment)
    db.session.commit()
    return jsonify(comment.to_json())
